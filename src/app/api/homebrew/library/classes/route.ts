...
import { listAvailableClasses } from "@/lib/homebrew/homebrewStore";
...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ classes: listAvailableClasses(userId, ruleset as RulesetId | undefined) });
  } catch (error) {
    return homebrewErrorResponse(error) ?? serverError(error, "Could not list homebrew classes.");
  }
}
