...
export default function CampaignHandoutShareModal(props: {
  ...
  return <div className="modal-scrim" role="presentation" onMouseDown={props.onClose}>
    <section className="campaign-handout-share" role="dialog" aria-modal="true" aria-labelledby="campaign-handout-share-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="campaign-handout-share-status"><span className={props.handout.shared ? "is-shared" : "is-private"}>{props.handout.shared ? "Shared" : "Private"}</span><p>{props.handout.shared ? "This handout is already shared. Choose a new audience to share it again." : "Only you can see this handout until you share it."}</p></div>
      <div className="campaign-handout-share-options"><div className="campaign-handout-share-group"><span className="campaign-handout-share-label">Campaign audience</span><button type="button" className="ao-cw-btn ao-cw-btn-primary" disabled={busy} onClick={() => void share(null)}><Share2 size={15} /> Share with everyone</button></div><div className="campaign-handout-share-group"><span className="campaign-handout-share-label">Specific player</span>{players.length ? <div className="campaign-handout-player-options">{players.map((member) => <button key={member.userId} type="button" className="ao-cw-btn" disabled={busy} onClick={() => void share(member.userId)}>{member.characterName ?? member.userName}</button>)}</div> : <p className="campaign-handout-share-empty">No other registered players are currently in this campaign.</p>}</div></div>
