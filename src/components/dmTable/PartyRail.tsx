...
export default memo(function PartyRail({ members, dmUserId, selectedUserId, currentTurnUserId, compact, onSelect, onOpenSheet }: Props) {
            ...
            <div key={member.userId} className={`dm-command-member is-${state}${selected ? " is-selected" : ""}${current ? " is-current" : ""}`}>
              <button type="button" role="option" aria-selected={selected} onClick={() => onSelect(member)}>
                <span className="dm-command-member-name">
                  ...
                  <small>{member.characterClass ? `${member.characterClass} ${member.characterLevel ?? ""}` : "Character not loaded"}</small>
                </span>
                {!compact ? (
                  <>
