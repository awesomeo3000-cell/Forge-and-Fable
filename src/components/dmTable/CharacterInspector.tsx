    ...
    <aside className="dm-command-inspector" aria-label={`${member.characterName ?? member.userName} inspector`}>
      ...
      <div className="dm-command-inspector-body" role="tabpanel">
        {tab === "overview" ? <div className="dm-command-overview">
          ...
          {member.conditions.length ? <section><h4>Conditions</h4><div className="dm-command-tags">{member.conditions.map((condition) => <ConditionChip key={condition} label={condition} />)}</div></section> : null}
          {member.concentratingOn ? <section><h4>Concentration</h4><ConditionChip label={member.concentratingOn} concentration />{onCharacterAction ? <div className="dm-inspector-actions"><button type="button" className="dm-btn" onClick={() => { const value = window.prompt("Damage from this hit", "10"); if (value) onCharacterAction(member, "concentration-check", Number(value)); }}>Request check</button><button type="button" className="dm-btn" onClick={() => onCharacterAction(member, "concentration-end")}>End concentration</button></div> : null}</section> : null}
          ...
          {member.spellSlots.length ? <section><h4>Spell slots</h4><SpellSlotTrack slots={member.spellSlots} /></section> : null}
          {resources.length ? <section><h4>Resources</h4><div className="dm-inspector-resources">{resources.map((resource) => <ResourceChip key={resource.id} resource={resource} detailed />)}</div></section> : null}
        </div> : null}
