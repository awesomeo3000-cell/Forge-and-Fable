...
export default function CampaignMemoryPanel({campaignId}:{campaignId:string}){
  ...
  useEffect(()=>{let cancelled=false;void fetch(`/api/campaigns/${campaignId}/memory`).then(async(response)=>{const data=await response.json().catch(()=>null) as PlayerCampaignMemory & {error?:string}|null;if(!response.ok)throw new Error(data?.error??"Could not load campaign memory.");if(!data)throw new Error("Could not load campaign memory: server returned an invalid response.");if(!cancelled)setMemory(data);}).catch((reason)=>!cancelled&&setError(reason instanceof Error?reason.message:"Could not load campaign memory."));return()=>{cancelled=true};},[campaignId]);
