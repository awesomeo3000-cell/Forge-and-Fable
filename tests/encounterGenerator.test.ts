...
describe("encounter preparation rules",()=>{
  ...
  it("warns about flight and large hits",()=>{const warnings=encounterSafetyWarnings(party,[{creature:BUILT_IN_CREATURES.find((item)=>item.id==="builtin:air-elemental")!,quantity:2}]);expect(warnings.map((item)=>item.code)).toEqual(expect.arrayContaining(["one-hit","flying","area-damage"]));});
